...
int main()
{
	...
	//model = glm::translate(model, glm::vec3(300.0, 0, 0));
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (int i = 1; i < 4; i++) {
			glm::mat4 model = glm::mat4(1); // Matriz identidade
			model = glm::translate(model, glm::vec3(i * 150.0, i * 100.0, 0));
			model = glm::rotate(model, i * (float)glfwGetTime(), glm::vec3(0, 0, 1));
			model = glm::scale(model, glm::vec3(i / 4.0 + 0.5, i / 4.0 + 0.5, 1.0));
			model = glm::scale(model, glm::vec3(i / 2 + 0.5, i / 2 + 0.5, 1.0));
